...
import { setGlobalStateValue, setWorkspaceStorageValue, getGlobalStorageValue_sync, setGlobalStorageValue } from "./storage";
...
export async function initSyncProfile(ctx: vscode.ExtensionContext) {
  let inputGistkey;
  let placeHolder = "Come up with a gist key";
  while (true) {
    inputGistkey = await vscode.window.showInputBox({
      placeHolder,
      title: "Set gist key",
    });
    if (!inputGistkey) {
...
export async function uploadProfile(ctx: vscode.ExtensionContext) {
  ...
  //let input1 = "{\"extensions\":" + JSON.stringify(ctx.globalState.get("extensions"));
  //let input2 = ",\"profiles\":" + JSON.stringify(ctx.globalState.get("profiles")) + ",\"isHideMessageRestart\":true}";
  //console.log(input2);
  //syncwriteFile(input1 + input2);
  //console.log("continue");
  //console.log(input1 + input2);
}

export async function downloadProfile(ctx: vscode.ExtensionContext) {
  //let profile;
  var respData = getgist();
  //console.log("continue");
  ...
  respData.then(result => {
    ...
    if (findNestedProperty(jsonObject, "message") == "Bad credentials") {
      ...
      let extensions = findNestedProperty(jsonObject, "vscode-extensions-profiles.json").content;
      //profile = result.match(/{\\\"extensions\\\":.*}/g).toString();
      extensions = extensions.replaceAll("\\\"", "\"").toString();
